...
test("Should not create a user with invalid name", async function () {
    expect(() => new User_1.default("Churros", "churros@gmail.com", "churros123", 19)).toThrow(new Error("Invalid username"));
});
test("Should not create a user with invalid email", async function () {
    expect(() => new User_1.default("Churros Augusto", "churros@gmail", "churros123", 19)).toThrow(new Error("Invalid email"));
});
test("Should not create a user with invalid password", async function () {
    expect(() => new User_1.default("Churros Augusto", "churros@gmail.com", "chur", 19)).toThrow(new Error("Invalid password"));
});
test("Should not create a user with invalid age", async function () {
    expect(() => new User_1.default("Churros Augusto", "churros@gmail.com", "churros123", 8)).toThrow(new Error("Invalid age"));
});
